  ...
  config = function()
    require('typescript-tools').setup {
      on_attach = function(client)
        ...
        client.server.capabilities.documentFomrattingProvider = false
    ...
    vim.keymap.set('n', '<leader>lz', '<cmd>TSToolsGoToSourceDefinition<cr>', { desc = 'Go To Source Definition' })
    vim.keymap.set('n', '<leader>lR', '<cmd>TSToolsRemoveUnusedImports<cr>', { desc = 'Removed Unused Imports' })
